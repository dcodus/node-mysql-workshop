...
var query = 'select Account.id, Account.email, AddressBook.name from Account left outer join AddressBook on Account.id = AddressBook.accountId';
...
connection.query(query, function(err, response){
    ...
    response.forEach(function(oldAccount, index){
        ...
        if(!found){
            ...
            if(oldAccount.name === null){
               var account = {'id': oldAccount.id, 'email': oldAccount.email, 'books': ['--no address books--']}; 
